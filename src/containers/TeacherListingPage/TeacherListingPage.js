...
import { TeacherListingWizard, NamedRedirect, Page } from '../../components';
...
export const TeacherListingPageComponent = props => {
  const {
    currentUser,
  ...
  if (shouldRedirect) {
    ...
    // Images are passed to TeacherListingForm so that it can generate thumbnails out of them
    ...
    const title = isNewListingFlow
      ? intl.formatMessage({ id: 'TeacherListingPage.titleCreateListing' })
      : intl.formatMessage({ id: 'TeacherListingPage.titleEditListing' });
    ...
    const loadingPageMsg = {
      id: 'TeacherListingPage.loadingListingData',
    };
    return (
...
const mapStateToProps = state => {
  const page = state.TeacherListingPage;
  const {
    getAccountLinkInProgress,
    getAccountLinkError,
...
)(injectIntl(TeacherListingPageComponent));
