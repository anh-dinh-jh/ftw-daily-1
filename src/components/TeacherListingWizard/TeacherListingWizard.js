...
import TeacherListingWizardTab, {
  AVAILABILITY,
  GENERAL,
  LOCATION,
  PRICING,
  PHOTOS,
} from './TeacherListingWizardTab';
...
const tabLabel = (intl, tab) => {
  let key = null;
  if (tab === GENERAL) {
    key = 'TeacherListingWizard.tabLabelGeneral';
  } else if (tab === LOCATION) {
    key = 'TeacherListingWizard.tabLabelLocation';
  } else if (tab === PRICING) {
    key = 'TeacherListingWizard.tabLabelPricing';
  } else if (tab === AVAILABILITY) {
    key = 'TeacherListingWizard.tabLabelAvailability';
  } else if (tab === PHOTOS) {
    key = 'TeacherListingWizard.tabLabelPhotos';
  }
...
const RedirectToStripe = ({ redirectFn }) => {
  ...
  return <FormattedMessage id="TeacherListingWizard.redirectingToStripe" />;
};

// Create a new or edit listing through TeacherListingWizard
class TeacherListingWizard extends Component {
  constructor(props) {
    super(props);
  ...
  handlePayoutSubmit(values) {
    this.props
      ...
      .then(response => {
        this.props.onManageDisableScrolling('TeacherListingWizard.payoutModal', false);
      })
      .catch(() => {
        // do nothing
      });
  ...
  render() {
    ...
    if (!tabsStatus[selectedTab]) {
      ...
      return <NamedRedirect name="TeacherListingPage" params={{ ...params, tab: nearestActiveTab }} />;
    }
    ...
    const tabLink = tab => {
      return { name: 'TeacherListingPage', params: { ...params, tab } };
    };
    ...
    if (returnedNormallyFromStripe && stripeConnected && !requirementsMissing) {
      return <NamedRedirect name="TeacherListingPage" params={pathParams} />;
    }
    ...
    return (
      <div className={classes}>
        ...
        <Modal
          id="TeacherListingWizard.payoutModal"
        ...
        >
          <div className={css.modalPayoutDetailsWrapper}>
            <h1 className={css.modalTitle}>
              <FormattedMessage id="TeacherListingWizard.payoutModalTitleOneMoreThing" />
              <br />
              <FormattedMessage id="TeacherListingWizard.payoutModalTitlePayoutPreferences" />
            </h1>
            {!currentUserLoaded ? (
              ...
              <>
                <p className={css.modalMessage}>
                  <FormattedMessage id="TeacherListingWizard.payoutModalInfo" />
                </p>
...
TeacherListingWizard.defaultProps = {
  className: null,
  currentUser: null,
  rootClassName: null,
  listing: null,
...
TeacherListingWizard.propTypes = {
